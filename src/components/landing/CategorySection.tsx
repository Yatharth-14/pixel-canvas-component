...
import { getProductsByCategory, getCategories, Category, Product } from '@/Backend/services/products';
...
const CategoryItem: React.FC<CategoryItemProps> = ({ title, image, slug, items }) => {
  return (
      ...
      <CardContent className="p-4">
        <div className="flex items-center mb-3">
          ...
          <Link to={`/category/${slug}`}>
            <h3 className="font-semibold text-medical-primary hover:underline">{title}</h3>
          </Link>
        </div>
        <ul className="space-y-1.5">
          {items.slice(0, 4).map((item) => (
            <li key={item.id} className="text-sm text-gray-700 hover:text-medical-accent flex items-center">
...
const CategorySection: React.FC<CategorySectionProps> = ({ title, slug }) => {
  ...
  const getProductGroups = () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const groupProducts = products.slice(startIndex, startIndex + itemsPerGroup);
  ...
  const getGroupImage = (index: number) => {
    ...
    const groupProducts = products.slice(startIndex, startIndex + Math.ceil(products.length / 4));
    ...
    <section className="py-10 bg-gray-50">
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link to={`/category/${slug}`} className="text-medical-accent hover:underline text-sm flex items-center">
